...
// This part is defining the mirroring so that I didn't have to include 2x images in my sprite sheet
    ...
    function drawPlayer(ctx) {
        ...
        if (player.frameTimer >= player.frameDelay) {
            player.frameTimer = 0;  // Reset the timer
            player.currentFrame = (player.currentFrame + 1) % totalRunningFrames; // Update the frame
        }        
    }
    ...
    window.addEventListener('keydown', function(event) {
        if (event.key === 'ArrowLeft') {
            ...
            player.positionX -= player.speed;  // Move left
        } else if (event.key === 'ArrowRight') {
            player.state = 'running';
            player.direction = 'right';  // Set direction to right
            player.positionX += player.speed;  // Move right
        }
    });

    window.addEventListener('keyup', function(event) {
        if (event.key === 'ArrowLeft' || event.key === 'ArrowRight') {
            player.state = 'idle';  // Switch to idle state
            player.currentFrame = 0; // Reset animation to idle frame
        }
    });
